...
void SeaWood::Renderer::beginFrame()
{
...
void SeaWood::Renderer::setBackgroundColor(const FLOAT colorRGBA[4])
{
	for (int i = 0; i < 4; i++)
	{
...
void SeaWood::Renderer::initRenderTarget()
{
	_ASSERT(m_D3DImmediateContext);
	_ASSERT(m_D3DDevice);
	_ASSERT(m_SwapChain);
...
ID3D11Device* SeaWood::Renderer::getDevice()
{
	return m_D3DDevice;
}

void SeaWood::Renderer::setInputLayout(ID3D11InputLayout* inputLayout)
{
	m_D3DImmediateContext->IASetInputLayout(inputLayout);
}

void SeaWood::Renderer::setPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY topology)
{
	m_D3DImmediateContext->IASetPrimitiveTopology(topology);
}

ID3D11DeviceContext* SeaWood::Renderer::getDeviceContext()
{
